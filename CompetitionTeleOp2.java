...
//        }else{
            int sign = 1;
            if(frontLeftPowerFactor >= 0) sign = 1; else sign = -1;
            leftFrontDrive.setPower(sign*(frontLeftPowerFactor * magLeft)*(frontLeftPowerFactor * magLeft));
            if(frontRightPowerFactor >= 0) sign = 1; else sign = -1;
            rightFrontDrive.setPower(-sign*(frontRightPowerFactor * magRight)*(frontRightPowerFactor * magRight));
            if(backLeftPowerFactor >= 0) sign = 1; else sign = -1;
            leftBackDrive.setPower(sign*(backLeftPowerFactor * magLeft)*(backLeftPowerFactor * magLeft));
            if(backRightPowerFactor >= 0) sign = 1; else sign = -1;
            rightBackDrive.setPower(-sign*(backRightPowerFactor * magRight)*(backRightPowerFactor * magRight));
//        }
            ...
            Thread shooterThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    Load.setPosition(0.70);
                    //delay must add to 900ms
                    for(int i = 0; i < 5; i++) {
